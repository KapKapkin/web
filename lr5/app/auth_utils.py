...
def check_rights(permission):
    ...
    Args:
        permission (str): Требуемое право
...
def can_user_access(permission):
    """
    Вспомогательная функция для проверки прав в шаблонах
...
def can_edit_user(user_id):
    """
    Проверка права редактирования конкретного пользователя
    
    Args:
        user_id (int): ID пользователя для редактирования
        
    Returns:
        bool: True если можно редактировать
    """
    if not current_user.is_authenticated:
        return False
...
def can_view_user(user_id):
    """
    Проверка права просмотра профиля конкретного пользователя
    
    Args:
        user_id (int): ID пользователя для просмотра
        
    Returns:
        bool: True если можно просматривать
    """
    if not current_user.is_authenticated:
        return False
