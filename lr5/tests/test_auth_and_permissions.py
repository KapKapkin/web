...
@pytest.fixture
def runner(app):
    """Создаёт тестовый runner для CLI команд"""
...
class TestAuthorization:
    ...
    def test_admin_can_access_users_list(self, client):
        """Тест: администратор может просматривать список пользователей"""
        login_user(client, 'admin', 'admin123')
        rv = client.get('/users/')
        assert rv.status_code == 200
    ...
    def test_user_cannot_access_users_list(self, client):
        ...
        rv = client.get('/users/')
        assert rv.status_code == 302  # Редирект
    ...
    def test_user_cannot_create_user(self, client):
        """Тест: обычный пользователь не может создавать пользователей"""
        login_user(client, 'testuser', 'test123')
        rv = client.get('/users/create', follow_redirects=True)
        assert 'У вас нет прав' in rv.get_data(as_text=True)
    
    def test_admin_can_edit_any_user(self, client, app):
        """Тест: администратор может редактировать любого пользователя"""
        login_user(client, 'admin', 'admin123')
        with app.app_context():
    ...
    def test_user_cannot_delete_user(self, client, app):
        """Тест: обычный пользователь не может удалять пользователей"""
        login_user(client, 'testuser', 'test123')
        with app.app_context():
            user = User.query.filter_by(login='another').first()
            rv = client.post(f'/users/{user.id}/delete', follow_redirects=True)
...
class TestVisitLogging:
    ...
    def test_visit_logged_on_page_access(self, client, app):
        """Тест: посещения записываются в журнал"""
        login_user(client, 'testuser', 'test123')
        ...
        with app.app_context():
            ...
            assert len(visits) >= 3  # Как минимум 3 посещения
    
    def test_anonymous_visits_logged(self, client, app):
        """Тест: анонимные посещения записываются"""
        client.get('/')
    ...
    def test_visit_log_contains_correct_data(self, client, app):
        """Тест: журнал содержит корректные данные"""
        login_user(client, 'testuser', 'test123')
        client.get('/users/profile')
...
class TestReports:
    ...
    def test_admin_can_access_reports(self, client):
        """Тест: администратор может просматривать отчёты"""
        login_user(client, 'admin', 'admin123')
    ...
    def test_user_cannot_access_reports(self, client):
        """Тест: обычный пользователь не может просматривать отчёты"""
        login_user(client, 'testuser', 'test123')
        rv = client.get('/reports/', follow_redirects=True)
        assert 'У вас нет прав' in rv.get_data(as_text=True)
    
    def test_pages_stats_report(self, client, app):
        """Тест: отчёт по страницам работает корректно"""
        # Создаём данные для отчёта
        login_user(client, 'testuser', 'test123')
        client.get('/users/profile')
        client.get('/users/profile')
        client.get('/users/change_password')
        
        # Заходим как администратор и проверяем отчёт
        logout_user(client)
        login_user(client, 'admin', 'admin123')
    ...
    def test_users_stats_report(self, client, app):
        """Тест: отчёт по пользователям работает корректно"""
        # Создаём данные для отчёта
        login_user(client, 'testuser', 'test123')
        client.get('/users/profile')
        logout_user(client)
    ...
    def test_csv_export_pages(self, client, app):
        """Тест: экспорт статистики страниц в CSV"""
        # Создаём данные
        login_user(client, 'testuser', 'test123')
        client.get('/users/profile')
        logout_user(client)
        
        # Экспортируем как администратор
        login_user(client, 'admin', 'admin123')
        ...
        rows = list(reader)
        assert len(rows) >= 2  # Заголовок + как минимум одна строка данных
        assert rows[0] == ['Страница', 'Количество посещений']
    
    def test_csv_export_users(self, client, app):
        """Тест: экспорт статистики пользователей в CSV"""
        # Создаём данные
        login_user(client, 'testuser', 'test123')
        client.get('/users/profile')
        logout_user(client)
...
class TestUserManagement:
    ...
    def test_create_user_as_admin(self, client, app):
        """Тест: создание пользователя администратором"""
        login_user(client, 'admin', 'admin123')
    ...
    def test_edit_user_as_admin(self, client, app):
        """Тест: редактирование пользователя администратором"""
        login_user(client, 'admin', 'admin123')
...
class TestUserProfile:
    ...
    def test_user_can_view_own_visits(self, client, app):
        """Тест: пользователь может просматривать свои посещения"""
        login_user(client, 'testuser', 'test123')
        
        # Создаём несколько посещений
        client.get('/users/profile')
        client.get('/users/change_password')
